...
- provider-scoped model ID와 custom OpenAI-compatible endpoint
- global permission과 agent-specific permission
- `plan` agent와 `build` agent의 다른 permission envelope
- MCP server enable/disable, auth state, local/remote transport
...
-> preview
-> checkpoint
-> hash-validated create-only apply
```
...
Tinker.Gen
  ...
  - deterministic preview/checkpoint
