...
- 2026-07-03 관측에서 GitHub Environment `prod` raw API `protection_rules`에는 required reviewer `WonderRabbit`가 표시되었다. [RC-PROD-REVIEWER-20260703] 같은 관측에서 `can_admins_bypass: false` [RC-PROD-ADMIN-BYPASS-20260703], `deployment_branch_policy: null` [RC-PROD-BRANCH-POLICY-20260703]이었다. 관측의 GET 출처·2026-08-02 만료·읽기 전용 재검증 명령은 `plan/history/remote-claims.json`을 따른다. jq로 `.reviewers`만 조회하면 `null`일 수 있으므로 이 필드만으로 승인 규칙 부재를 판단하지 않는다. source-controlled local fact인 workflow의 `main`/`v*` branch guard는 유지한다.
...
- 2026-07-03 관측의 GitHub Environment `prod` required reviewer `WonderRabbit` [RC-PROD-REVIEWER-20260703] 승인 경계를 사용한다. 같은 관측의 `can_admins_bypass: false` [RC-PROD-ADMIN-BYPASS-20260703]는 2026-08-02에 만료되며, 이후에는 `plan/history/remote-claims.json`의 읽기 전용 GET 명령으로 재확인한다.
...
5. `prod` GitHub Environment는 2026-07-03 관측의 raw API required reviewer `WonderRabbit` [RC-PROD-REVIEWER-20260703] 승인 후 SSH deploy한다. 같은 관측의 `can_admins_bypass: false` [RC-PROD-ADMIN-BYPASS-20260703]는 2026-08-02까지의 관측이며, 출처와 읽기 전용 재검증은 `plan/history/remote-claims.json`을 따른다.
...
권장 여부: **현재 기본 추천안**. prod는 2026-07-03 관측의 required reviewer `WonderRabbit` [RC-PROD-REVIEWER-20260703]와 `workflow_dispatch`, `main`/`v*` tag guard를 함께 요구한다. 같은 관측의 `can_admins_bypass: false` [RC-PROD-ADMIN-BYPASS-20260703]와 `deployment_branch_policy: null` [RC-PROD-BRANCH-POLICY-20260703]은 2026-08-02 만료 뒤 `plan/history/remote-claims.json`의 GET 명령으로 재검증한다. Environment branch policy만으로 보호한다고 가정하지 않고 source-controlled workflow branch guard를 유지한다.
...
- prod 배포는 2026-07-03 관측의 required reviewer `WonderRabbit` [RC-PROD-REVIEWER-20260703]와 `workflow_dispatch`, `main`/`v*` tag guard, `confirm_prod=deploy-prod` 입력을 함께 요구한다. 같은 관측의 `can_admins_bypass: false` [RC-PROD-ADMIN-BYPASS-20260703]와 `deployment_branch_policy: null` [RC-PROD-BRANCH-POLICY-20260703]은 2026-08-02까지만 유효하고, 이후 `plan/history/remote-claims.json`의 읽기 전용 GET 명령으로 재검증한다. source-controlled workflow guard는 원격 관측과 별도로 완료 조건에 유지한다.
