...
Tomodachi의 현재 기본값은 **1안: GitHub Actions + GHCR + SSH/Compose**가 맞다. 이미 저장소에는 GitHub Actions workflow, GHCR image publish 흐름, dev/prod Compose overlay, SSH 기반 deploy workflow가 준비되어 있다. 2026-07-03 관측에서 GitHub Environment `prod`의 raw API `protection_rules`에는 required reviewer `WonderRabbit` [RC-PROD-REVIEWER-20260703] 규칙이 표시되었고, `can_admins_bypass: false` [RC-PROD-ADMIN-BYPASS-20260703], `deployment_branch_policy: null` [RC-PROD-BRANCH-POLICY-20260703]이었다. 이 관측은 2026-08-02에 만료되며 GET 출처와 읽기 전용 재검증 명령은 `plan/history/remote-claims.json`을 따른다.
...
- GitHub Environment: 2026-07-03 관측에서 `prod` raw API `protection_rules`에는 required reviewer `WonderRabbit` [RC-PROD-REVIEWER-20260703] 보호 규칙이 표시되었고, `can_admins_bypass: false` [RC-PROD-ADMIN-BYPASS-20260703], `deployment_branch_policy: null` [RC-PROD-BRANCH-POLICY-20260703]이었다. 2026-08-02 만료 뒤에는 `plan/history/remote-claims.json`의 GET 명령으로 재검증한다. jq로 `.reviewers`만 보면 `null`일 수 있으므로 이 필드만으로 승인 규칙 부재를 판단하지 않는다. source-controlled workflow branch guard는 유지한다.
...
| Prod 승인 | 2026-07-03 관측의 GitHub Environment `prod` required reviewer `WonderRabbit` [RC-PROD-REVIEWER-20260703]. 같은 관측의 `can_admins_bypass: false` [RC-PROD-ADMIN-BYPASS-20260703]는 2026-08-02 만료 뒤 inventory의 GET 명령으로 재검증 | GitHub Environment + cloud IAM/change control | Jenkins input step 또는 별도 approval plugin 필요 |
...
4. `prod`는 수동 dispatch만 허용하고, 2026-07-03 관측의 GitHub Environment required reviewer `WonderRabbit` [RC-PROD-REVIEWER-20260703] 승인 후 SSH/Compose deploy한다. 같은 관측의 `can_admins_bypass: false` [RC-PROD-ADMIN-BYPASS-20260703]는 2026-08-02 만료 뒤 inventory의 GET 명령으로 재검증한다.
...
- prod 승인 경계가 명확하다. 2026-07-03 관측의 GitHub Environment `prod` required reviewer `WonderRabbit` [RC-PROD-REVIEWER-20260703]와 source-controlled workflow branch guard를 함께 쓴다. 같은 관측에서 admin bypass는 꺼져 있었다. [RC-PROD-ADMIN-BYPASS-20260703] 이 관측은 2026-08-02 만료 뒤 inventory의 GET 명령으로 재검증한다.
...
- 2026-07-03 관측에서 environment branch policy는 null이었다. [RC-PROD-BRANCH-POLICY-20260703] 2026-08-02 만료 뒤 inventory의 GET 명령으로 재검증하며, `.github/workflows/deploy-prod.yml`의 `main`/`v*` tag guard는 제거하지 않는다.
- 2026-07-03 관측의 `can_admins_bypass: false` [RC-PROD-ADMIN-BYPASS-20260703]와 `deployment_branch_policy: null` [RC-PROD-BRANCH-POLICY-20260703]은 2026-08-02 만료 뒤 inventory의 GET 명령으로 재검증한다. source-controlled workflow branch guard가 prod 배포 범위를 계속 제한해야 한다.
...
- Human approval: 2026-07-03 관측의 GitHub Environment `prod` required reviewer `WonderRabbit` [RC-PROD-REVIEWER-20260703] 승인. 같은 관측의 `can_admins_bypass: false` [RC-PROD-ADMIN-BYPASS-20260703]는 2026-08-02 만료 뒤 inventory의 GET 명령으로 재검증한다.
